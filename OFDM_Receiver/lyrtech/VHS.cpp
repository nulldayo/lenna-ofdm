...
#define versionstring "VHS.cpp, v0.8, date %s, %s %s\n"
...
void mexFunction(int n_output, mxArray *output[], int n_input, const mxArray *input[]){
    ...
    static ULONG ulTransmode   = -1; //-1 -->no mode, 0 --> RX, 1 --> TX
    static int pabs            = -1;
    static int dsw             = -1;
    static int oscs            = -1;
...
//---------------------------------------------------------------------------
        case READ_SDRAM_ADC:{
            ...
            buffer_in = (PUCHAR) malloc( (((length+4095) & ~4095) )*2 );
            ...
            bytes_read = Vhsadac16_Sdram_Read( m_h_board_adc, 0, buffer_in, length);
...
//---------------------------------------------------------------------------               
        case TRANS_MODE:{
            ...
            ulTransmode = (int)mxGetScalar(input[1]);
        ...
        case INIT_SPI_REGISTER:{
            ...
            if (n_input == 1){
                ...
                for (i=0; i < NUM_OF_TRANS; i++){
                    ...
                    SetChannel(ch, band, transceiver_number_tab[i], ulTransmode);
                }
              ...
              i = (int)mxGetScalar(input[1]);
                ...
                for (i2=2; i2<13;i2++){
                    ...
                    SetChannel(ch, band, transceiver_number_tab[i-1], ulTransmode);
...
//---------------------------------------------------------------------------   
        ...
        case SET_CHANNEL:{
            ...
            if (n_input == 3){
                ...
                SetChannel(channel, band, transceiver_number_tab[i-1], ulTransmode);          
            }else{
                for (i = 0; i < NUM_OF_TRANS; i++){
                    SetChannel(channel, band, transceiver_number_tab[i], ulTransmode);
                }
            }
            break;
        }
        ...
        case SET_RX_LOW_PASS_FILTER:
        {
          int imin,imax;
            ...
            if( n_input == 3)
            {
              ...
              if( (imin < 0) || (imax>4) )
              {
                printf("Transceiver number %d out of range ",imax);
